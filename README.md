...
使用 Arduino 框架，在 GPIO1 连续采集电位器电压。三个业务任务分别负责 ADC 处理、240×240 ST7789 显示、串口波形传输和状态统计。

## 电脑端实时波形

已提供 Python 中文上位机，使用 USB 串口接收与小屏幕同源的波形快照。支持实时电压、RAW 值、最小/最大电压包络、暂停显示、当前快照 CSV 导出、断流提示和无硬件演示。

在项目目录运行：

```powershell
# 先用 PlatformIO 上传新版固件（旧固件没有波形数据协议）。
pio run --target upload

# Python 3.10 或更新版本，需包含 tkinter；首次使用安装依赖。
py -3 -m pip install -r desktop/requirements.txt
py -3 desktop/wave_viewer.py
```

也可双击 `desktop/start.bat` 启动。选择开发板 COM 端口，点击“连接”；波特率固定为 **115200**。先关闭 PlatformIO 串口监视器，烧录前也先断开上位机。没有开发板时可点击“演示模式”，或运行：

```powershell
py -3 desktop/wave_viewer.py --demo
```

电脑端目标约 **10 次/秒** 更新，显示最近 **2.16 秒**、每桶 **10ms** 的校准电压最小/最大值。完整快照重复覆盖历史，因此跳过旧显示更新不会造成历史重复拼接。它同步的是波形包络，并不上传每秒 4000 个原始采样点；横轴相对于最新完整时间桶，固件时间戳是处理结果生成时间。小屏幕仍以目标 20 FPS 独立刷新，两端不保证同一时刻显示同一帧。

“暂停”仅冻结显示，后台继续接收；“继续”立即显示最新快照。“导出当前波形”保存屏幕上的一份快照，含每桶起止相对时间、最小/最大毫伏值、平均 RAW 值、滤波电压和设备时间戳，不是连续录制。演示导出的文件名前缀为 `demo_`。

串口断开时提示重新连接，超过 600ms 未收到帧时清空实时曲线。固件另报告采样数据过期及 ADC 超时/恢复/错误状态。连接后一直没有曲线时，检查是否已上传新版固件及选择正确端口；原生 USB 口的配置见下文“编译与上传”。新版串口同时含二进制波形和文本统计，普通文本串口监视器出现乱码属于二进制数据的显示结果。

实现、协议与测试命令见 [上位机说明](desktop/README.md)。
...
| StatusTask | 1（低） | Core 1 | 3072 字节 | 每 100ms 发送波形快照，每约 1000ms 打印统计 |
...
- ADCProcessTask 另将相同快照写入 `telemetryQueue`，StatusTask 独立读取并串口发送；不等待串口，也不抢走屏幕数据。上图省略了这条独立通路。
...
当前已启用 `ARDUINO_USB_MODE=1` 和 `ARDUINO_USB_CDC_ON_BOOT=1`，`Serial` 波形通过 ESP32-S3 原生 USB Serial/JTAG 接口发送。请将数据线插在板上标注 **USB** 的接口，烧录后在上位机刷新并选择该接口对应的 COM 端口。无需同时连接 COM/UART 接口；烧录或复位后端口编号可能变化。若以后改用板上的 USB 转串口（COM/UART）接口接收波形，将 `ARDUINO_USB_CDC_ON_BOOT` 改为 `0`，重新编译烧录。
...
- 含电脑端波形传输、启用原生 USB CDC 的固件编译成功：静态 RAM 使用 23,844 字节，程序使用 299,909 字节。
- 上位机 7 项测试通过，覆盖协议交叉解码、损坏恢复、CSV、断线处理及隐藏窗口 GUI 检查。
